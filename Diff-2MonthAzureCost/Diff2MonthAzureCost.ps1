...
function addM1DataToResult ($m1Data, $m1DiffedData) {
	...
	$m1Data | ForEach-Object {
		...
		if ($m1DiffedData | Where-Object -Property $groupingBy -eq $m1Id) {
			#if $m1Id is already in $diffedData, add M1 Cost to $m1diffedData
			($m1DiffedData | Where-Object -Property $groupingBy -eq $m1Id)."Cost : ${m1YM}" += $m1Cost
			($m1DiffedData | Where-Object -Property $groupingBy -eq $m1Id)."Diff (${m2YM} - ${m1YM})" -= $m1Cost
		}
		else {
			#if $m1Id is not in $diffedData, add M1 Cost to $m1diffedData as new object
			$m1DiffedData += New-Object PSObject -Property @{
				$groupingBy                = $m1Id
				"Cost : ${m2YM}"           = 0
				"Cost : ${m1YM}"           = $m1Cost
				"Diff (${m2YM} - ${m1YM})" = 0 - $m1Cost
				Currency                   = $m1Currency
			}
		}
	}
...
function addM2DataToResult ($m2Data, $resultData) {
	...
	$m2Data | ForEach-Object {
		...
		if ($resultData | Where-Object -Property $groupingBy -eq $m2Id) {
			# if $m2Id is already in $diffedData, add M2 Cost to $diffedData and calculate diff
			($resultData | Where-Object -Property $groupingBy -eq $m2Id)."Cost : ${m2YM}" += $m2Cost
			($resultData | Where-Object -Property $groupingBy -eq $m2Id)."Diff (${m2YM} - ${m1YM})" += $m2Cost
		}
		elseif ($null -eq ($resultData | Where-Object -Property $groupingBy -eq $m2Id)) {
			# if $m2Id is not in $diffedData, add M2 Cost to $diffedData
			$resultData += New-Object PSObject -Property @{
				$groupingBy                = $m2Id
				"Cost : ${m2YM}"           = $m2Cost
				"Cost : ${m1YM}"           = 0
				"Diff (${m2YM} - ${m1YM})" = $m2Cost
				Currency                   = $m2Currency
			}
		}
	}
...
$m2YM = [Microsoft.VisualBasic.Interaction]::InputBox( "比較対象となる年月 (M2) をスラッシュ区切りで指定`r`n例 : 2024/1`r`n出力結果は M2 - M1 で算出`r`n", "比較対象となる年月の指定")

# Get Month1 Data : two-dimensional array
$m1YM = [Microsoft.VisualBasic.Interaction]::InputBox( "比較元となる年月 (M1) をスラッシュ区切りで指定`r`n例 : 2023/12`r`n出力結果は M2 - M1 で算出`r`n`r`nM2 の指定 : ${m2YM}", "比較元となる月の指定")
...
#Add M2 Data To Result and Calculate Diff
$result = addM2DataToResult $month2Data $m1Result

#Show Result
if ($showResult) {
	Write-Host "Show Result`n"
	$result | Select-Object -Property $groupingBy, "Cost : ${m2YM}", "Cost : ${m1YM}", "Diff (${m2YM} - ${m1YM})", Currency `
	| Sort-Object -Property "Diff (${m2YM} - ${m1YM})" -Descending | Out-GridView -Title "月額料金の差額 (${m2YM} - ${m1YM})"
}
